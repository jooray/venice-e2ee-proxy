...
curl http://127.0.0.1:3000/v1/chat/completions \
  ...
  -d '{"model": "e2ee-glm-5-2-p", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
curl http://127.0.0.1:3000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model": "e2ee-glm-5-2-p", "messages": [{"role": "user", "content": "Hello"}], "stream": true}'
```
...
### Andrew Miller's private-inference registry and audits

Everything above and in [what is actually attested](#what-is-actually-attested) looks at one provider. If you want to know how Venice compares to the alternatives, Andrew Miller ([@amiller](https://github.com/amiller), [@socrates1024](https://x.com/socrates1024)) maintains [awesome-private-inference](https://github.com/amiller/awesome-private-inference), a registry that re-probes attestation bundles from Venice, NEAR AI, Tinfoil, Chutes and RedPill/Phala daily and publishes the results as a [live scorecard](https://amiller.github.io/awesome-private-inference/). Each row records which verification layers a provider actually exposes and which it skips, so "has an attestation endpoint" does not get read as "private". No provider on it is complete today.

Venice currently reads Stage 1 with a verifying proxy and Stage 0 for the infrastructure and for Venice's own agent skill. This proxy is one of the third-party clients the registry credits with closing the client-side gap, by binding the workload keyset to a DCAP-verified quote rather than pinning it on first sight and failing closed when that check does not pass. The infrastructure row is held down by the two findings in [`docs/SECURITY-AUDIT.md`](docs/SECURITY-AUDIT.md): an operator root-SSH key that attestation cannot rule out on an unpublished dev OS image, and a last hop that holds your plaintext without its serving code being measured. The registry added those two layers to Venice's required set on 2026-08-10 after that audit pointed at them.

The reasoning behind each verdict lives in [devproof-audits-guide](https://github.com/amiller/devproof-audits-guide/tree/main/case-studies), a framework for the narrower question of whether the developer of a TEE service can quietly change, censor or exfiltrate from their own deployment. Its [Venice case study](https://github.com/amiller/devproof-audits-guide/blob/main/case-studies/venice-private-inference/DEVPROOF-REPORT.md) audits the surface this proxy exists to cover: the wire protocol checks out under live probing, but [`veniceai/skills`](https://github.com/veniceai/skills), which an agent loads to learn how to talk to Venice, misnames the protocol as HPKE or Noise when it is ECIES, cites a `docs.venice.ai/e2ee` URL that 404s, and teaches none of the verification steps. An agent following it trusts whatever signing key comes back. Miller calls that a backdoor by inattention rather than by design, which is a fair description and also a good reason to run a client that checks. Code there is MIT, the prose and scorecards CC-BY 4.0.
